...
$PAGE->set_url('/local/scormcreator/pagecontext_form.php', array('imsid' => $imsid));
 ...
 */
class local_scormcreator_pagecontext_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('filemanager', 'transcript', get_string('transcript', 'local_scormcreator'), null,
                           $file1options);
        $mform->setType('transcript', PARAM_RAW);
        $mform->addHelpButton('transcript', 'transcript', 'local_scormcreator');

        // Get session title from manifest.php.
        ...
        $repeatarray[] = $mform->createElement('header', 'pageloopheader', get_string('pagetitleno', 'local_scormcreator'));
...
// Save the files.
$file1options = array('subdirs' => 0, 'maxbytes' => '', 'context' => $context, 'accepted_types' =>
                array('.webm', '.ogg', '.pdf', '.docx', '.txt'), 'return_types' => FILE_INTERNAL | FILE_EXTERNAL );
$file2options = array('subdirs' => 0, 'maxbytes' => '', 'maxfiles' => 1, 'context' => $context, 'accepted_types' =>
                array('video/mp4'), 'return_types' => FILE_INTERNAL | FILE_EXTERNAL );
$file3options = array('subdirs' => 0, 'maxbytes' => '', 'maxfiles' => 1, 'context' => $context, 'accepted_types' =>
                array('webvtt/vtt'), 'return_types' => FILE_INTERNAL | FILE_EXTERNAL );

$mform = new local_scormcreator_pagecontext_form();
...
if ($mform->is_cancelled()) {
    ...
    if ($data = $mform->get_data()) {
        ...
        foreach ($getfiles as $gf) {
            file_save_draft_area_files($gf->transcript, $context->id, 'local_scormcreator', 'transcript', '0', $file1options);
