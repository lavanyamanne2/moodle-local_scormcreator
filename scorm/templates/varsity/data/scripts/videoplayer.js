(function ($) {
  ...
  window.AblePlayer = function(media) {
    ...
    // Eg., if chapters or transcripts are provided in an external container).
    ...
    /* Transcripts.
       ...
       3. "popup" - Automatically generated, written to a draggable, resizable popup window that can be toggled on/off
          with a button.
    ...
    /* In "Lyrics Mode", line breaks in WebVTT caption files are supported in the transcript.
       If false (default), line breaks are are removed from transcripts in order to provide a more seamless reading
        experience.
  ...
  AblePlayer.localGetElementById = function(element, id) {
    ...
    {
      /* If getRootNode is not supported it should be safe to use document.getElementById.
         (since there is no shadow DOM support).
      */
...
(function ($) {
  ...
  AblePlayer.prototype.getRootPath = function() {
    ...
    // Returns Able Player root path (assumes videoplayer.js is in /build, one directory removed from root).
    var scripts, i, scriptSrc, scriptFile, fullPath, ablePath, parentFolderIndex, rootPath;
    scripts= document.getElementsByTagName('script');
    for (i=0; i < scripts.length; i++) {
      scriptSrc = scripts[i].src;
      scriptFile = scriptSrc.substr(scriptSrc.lastIndexOf('/'));
      if (scriptFile.indexOf('videoplayer') !== -1) {
...
//    This.setDimensions();.
    ...
    playerPromise.done(
      function () {
        ...
        /* Go ahead and load media, without user requesting it.
           ...
           TODO: Confirm that this is still necessary with iOS (this would added early, & I don't remember what the
           issues were).
        */
  ...
  AblePlayer.prototype.setSeekInterval = function () {
    ...
    if (typeof duration === 'undefined' || duration < 1) {
      ...
      if (duration <= 20) {
         this.seekInterval = 5;  // Four steps max.
      }
      else if (duration <= 30) {
         this.seekInterval = 6; // Five steps max.
      }
      else if (duration <= 40) {
         this.seekInterval = 8; // Five steps max.
      }
      else if (duration <= 100) {
         this.seekInterval = 10; // Ten steps max.
      }
      else {
    ...
    $.ajax({
      ...
      success: function( data, textStatus, jqXHR) {
        ...
        */
        ...
        // Rremove the media element - we're done with it.
...
(function ($) {
  ...
  AblePlayer.prototype.getPreferencesGroups = function() {
    ...
    // Return array of groups in the order in which they will appear in the Preferences popup menu.
  ...
  AblePlayer.prototype.injectPrefsForm = function (form) {
    ...
    if (form == 'captions') {
      ...
      // Intro: introText += ' ' + this.tt.prefCookieWarning;.
      $prefsIntro = $('<p>',{
        text: introText
      });
    ...
    /* Code: $prefsDiv (dialog) must be appended to the BODY!
       otherwise when aria-hidden="true" is applied to all background content
       that will include an ancestor of the dialog,
       which will render the dialog unreadable by screen readers.
    */
    $('body').append($prefsDiv);
    dialog = new AccessibleDialog($prefsDiv, this.$prefsButton, 'dialog', formTitle, $prefsIntro, 
             thisObj.tt.closeButtonLabel, '32em');
...
(function ($) {
  ...
  function getCuePayload(state) {
    ...
    var result = {type: 'internal', tagName: '', value: '', classes: [], annotation: '', parent: null, children: [],
                  language: ''};
    var current = result;
    var languageStack = [];
...
(function ($) {
  ...
  AblePlayer.prototype.injectPlayerCode = function() {
    ...
    */
    ...
    // Code: if (this.player !== 'youtube') {.
      this.$ableWrapper.css({
        'max-width': this.playerMaxWidth + 'px'
      });
    // Close : } // End if not youtube.
...
if (this.mediaType === 'video' && this.hasChapters && this.useChaptersButton) {
  ...
  AblePlayer.prototype.addControls = function() {
    ...
    */
    ...
    i, j, k, controls, $controllerSpan, $sliderDiv, sliderLabel, duration, $pipe, $pipeImg, tooltipId,
    tooltipX, tooltipY, control,
    ...
    for (i = 0; i <= 3; i++) {
      ...
      for (j=0; j<controls.length; j++) {
        ...
        if (control === 'seek') {
          ...
          this.seekBar = new AccessibleSlider(this.mediaType, $sliderDiv, 'horizontal', baseSliderWidth, 0,
                         duration, this.seekInterval, sliderLabel, 'seekbar', true, 'visible');
        }
...
/*
  ...
  AblePlayer.prototype.swapSource = function(sourceIndex) {
    ...
    */
    ...
    // Code: $('.able-playlist li').removeClass('able-current');.
...
(function ($) {
  ...
  window. AccessibleSlider = function(mediaType, div, orientation, length, min, max, bigInterval, label, className,
                             trackingMedia, initialState) {
    var thisObj;
  ...
  // TODO: Native HTML5 can have several buffered segments, and this actually happens quite often.
  // Change this to display them all.
...
(function ($) {
  ...
  window.AccessibleDialog = function(modalDiv, $returnElement, dialogRole, title, $descDiv, closeButtonLabel, width,
                            fullscreen, escapeHook) {
    ...
    */
    ...
    if (this.player === 'html5') {
      ...
      if (this.usingAudioDescription()) {
        ...
        for (i=0; i < this.$sources.length; i++) {
          // Code: for all <source> elements, replace src with data-orig-src.
  ...
  */
  AblePlayer.prototype.getPlayerState = function () {
    ...
    if (this.player === 'html5') {
      ...
      if (this.jwPlayer.getState() === 'PAUSED' || this.jwPlayer.getState() === 'IDLE' ||
          this.jwPlayer.getState() === undefined) {
  ...
  AblePlayer.prototype.isPlaybackRateSupported = function () {
    if (this.player === 'html5') {
      ...
      /* Not directly supported by JW player; can hack for HTML5 version by finding the dynamically 
         generated video tag, but decided not to do that.*/
      return false;
    }
    else if (this.player === 'youtube') {
  ...
  AblePlayer.prototype.refreshControls = function() {
    ...
    if (this.stoppingYouTube) {
      ...
      if (typeof this.$status !== 'undefined' && typeof this.seekBar !== 'undefined') {
        ...
        /* Also don't update while tracking, since this may Pause/Play the player but we don't want to 
           send a Pause/Play update.*/
    ...
    if (this.$fullscreenButton) {
      if (!this.isFullscreen()) {
        ...
        if (this.iconType === 'font') {
          this.$fullscreenButton.find('span').first().removeClass('icon-fullscreen-collapse').
        ...
        if (this.iconType === 'font') {
          this.$fullscreenButton.find('span').first().removeClass('icon-fullscreen-expand').
    ...
    /* Update buffering progress.
       TODO: Currently only using the first HTML5 buffered interval, but this fails sometimes when buffering is
...
AblePlayer.prototype.handleCaptionToggle = function() {
    ...
    if (this.hidingPopup) {
      // Stop gap to prevent spacebar in Firefox from reopening popup.
  ...
  AblePlayer.prototype.setFullscreen = function (fullscreen) {
    ...
    if (this.nativeFullscreenSupported()) {
      ...
      if (!this.fullscreenDialog) {
        ...
        this.fullscreenDialog = new AccessibleDialog($dialogDiv, this.$fullscreenButton, 'dialog',
                                'Fullscreen video player', $fsDialogAlert, this.tt.exitFullScreen, '100%', true,
                                function () { thisObj.handleFullscreenToggle() });
...
(function ($) {
  ...
  AblePlayer.prototype.stylizeCaptions = function($element, pref) {
    /* The $element is the jQuery element containing the captions
...
(function ($) {
  ...
  AblePlayer.prototype.injectTranscriptArea = function() {
    ...
    // Add a default "Unknown" option; this will be deleted later if there are any elements with a language.
  ...
  AblePlayer.prototype.addTranscriptAreaEvents = function() {
    ...
    this.$transcriptDiv.on('mousewheel DOMMouseScroll click scroll', function (e) {
      ...
      */
      ...
      // Booboo.
      thisObj.scrollingTranscript = true;
    });
  ...
  AblePlayer.prototype.transcriptSrcHasRequiredParts = function() {
    ...
    /* Check the external transcript to be sure it has all required components.
       return true or false in the process, define all the needed variables and properties.
    */
  ...
  AblePlayer.prototype.setupManualTranscript = function() {
    ...
    this.$autoScrollTranscriptCheckbox = $('<input id="autoscroll-transcript-checkbox" type="checkbox">');
    this.$transcriptToolbar.append($('<label for="autoscroll-transcript-checkbox">' + this.tt.autoScroll + ': </label>'),
...
(function ($) {
  ...
  AblePlayer.prototype.addEventListeners = function () {
    ...
    // Handle mouse movement over player; make controls visible again if hidden booboo.
...
(function ($) {
  ...
  AblePlayer.prototype.addWindowMenu = function(which, $window, windowName) {
    ...
    */
    ...
    // For now, re-use preferences icon for this purpose booboo.
  ...
  AblePlayer.prototype.addResizeDialog = function (which, $window) {
    ...
    */
    ...
    resizeDialog = new AccessibleDialog($resizeForm, $windowButton, 'alert', this.tt.windowResizeHeading, $resizeWrapper,
                  this.tt.closeButtonLabel, '20em');
    if (which === 'transcript') {
...
(function ($) {
  AblePlayer.prototype.injectVTS = function() {
    ...
    if ($('#able-vts').length) {
      ...
      if (this.vtsTracks.length) {
        ...
        // TODO: Localize; intelligently assign proper heading level.
  ...
  AblePlayer.prototype.adjustTimes = function(rowNum) {
    ...
    if (isNaN(start)) {
      if (prevKind == null) {
        // The previous row was probably inserted, and user has not yet selected a kind automatically set it to captions
        prevKind = 'captions';
